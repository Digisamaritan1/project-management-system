...
export const EditProjectName = (ProjectNameobj) => {
    return `<p>Project name is changed from <strong> ${sanitizeInput(ProjectNameobj.previousTaskName)}</strong> to <strong> ${ProjectNameobj.TaskName} </strong>.</p>`;
}
...
export const projectStartDateAdd = (obj) => {
    return `<p>Start Date of <strong>${sanitizeInput(obj.ProjectName)}</strong> project is added as <strong><span style="background-color: rgb(236 238 255);color: #2F3990;border-radius: 5px;padding-right: 5px;padding-left: 5px;">${obj.formetedStartDate}</span></strong>.</p>`;
}
...
export const projectEndDateAdd = (obj) => {
    return `<p>End Date of <strong>${sanitizeInput(obj.ProjectName)}</strong> project is added as <strong><span style="background-color: rgb(236 238 255);color: #2F3990;border-radius: 5px;padding-right: 5px;padding-left: 5px;">${obj.formatedDate}</span></strong>.</p>`;
}
...
export const projectDueDateAdd = (obj) => {
    return `<p>Due Date of <strong>${sanitizeInput(obj.ProjectName)}</strong> project is added as <strong><span style="background-color: rgb(236 238 255);color: #2F3990;border-radius: 5px;padding-right: 5px;padding-left: 5px;">${obj.changedDate}</span></strong>.</p>`;
}
...
export const projectAttachmentAdd = (attachObj) => {
    return `<p><strong>${attachObj.url}</strong> attached on <strong>${sanitizeInput(attachObj.ProjectName)}</strong> project.</p>`;
}
...
export const taskStartAndDueDateAdd = (obj) => {
    return `<p>In <strong>${sanitizeInput(obj.ProjectName)}</strong> Project, Start And Due Date of <strong>${sanitizeInput(obj.TaskName) ? sanitizeInput(obj.TaskName) : ""
        }</strong> is added as <strong><span style="background-color: rgb(236 238 255);color: #2F3990;border-radius: 5px;padding-right: 5px;padding-left: 5px;">${obj.lastDate}</span></strong>.</p>`;
}
...
export const taskDescriptionAdd = (Descobj) => {
    return `<p>In <strong>${sanitizeInput(Descobj.ProjectName)}</strong> Project, Description of <strong>${sanitizeInput(Descobj.TaskName)
...
export const taskDescriptionChange = (Descobj) => {
    return `<p>In <strong>${sanitizeInput(Descobj.ProjectName)}</strong> Project, Description of <strong>${sanitizeInput(Descobj.TaskName)}</strong> is changed from <strong>"${Descobj.previousDiscriptionText.length > 20 ? Descobj.previousDiscriptionText.slice(0, 20) + '...' : Descobj.previousDiscriptionText}"</strong> to <strong>"${Descobj.textSimple.length > 20 ? Descobj.textSimple.slice(0, 20) + '...' : Descobj.textSimple}"</strong>.</p>`;
}

// For Task Attachment Add //
export const taskAttachmentAdd = (Attachobj) => {
    return `<p>In <strong>${sanitizeInput(Attachobj.ProjectName)}</strong> Project, <strong>${Attachobj.url}</strong> attached on <strong>${sanitizeInput(Attachobj.TaskName)}</strong>.</p>`;
}

// For Task Attachment Remove //
export const taskAttachmentRemove = (Attachobj) => {
    return `<p>In <strong>${sanitizeInput(Attachobj.ProjectName)}</strong> Project, <strong>${Attachobj.removeFileName}</strong> removed on <strong>${sanitizeInput(Attachobj.TaskName)}</strong>.</p>`;
}

// For Task Checklist Add //
export const taskCheckList = (Checkobj) => {
    return `<p>In <strong>${sanitizeInput(Checkobj.ProjectName)}</strong> Project, <strong>${sanitizeInput(Checkobj.TaskName)}</strong> Created a new Check List named <strong>${Checkobj.value}</strong>.</p>`;
}
...
export const taskCheckListEdit = (checkObj) => {
    return `<p>Project <strong>${sanitizeInput(checkObj.ProjectName)}</strong> Updated a  Check List named  from <strong> ${checkObj.previouName}</strong> to  <strong>${checkObj.newName}</strong> in <strong>${sanitizeInput(checkObj.TaskName)}</strong>.</p>`;
}

export const taskCheckListRemove = (checkObj) => {
    return `<p>Project <strong>${sanitizeInput(checkObj.ProjectName)}</strong> Removed a  ${checkObj.checklist} <strong> ${checkObj.name}</strong> from <strong>${sanitizeInput(checkObj.TaskName)}</strong>.</p>`;
}
