<template>
    ...
    <Template v-if="isVerify === false">
        <div class="ah-rightside">
            <div>
                <h3 v-if="!messageText" class="title-login font-weight-bold text-capitalize dark-gray">{{$t('Auth.set_new_password')}}</h3>
            ...
            <form action="#" @submit.prevent="handleSubmit" v-if="!showResend">
                ...
                <div class="form-group">
                    <button v-if="!isSpinner" class="btn btn-blue btn-login font-roboto-sans bg-blue white cursor-pointer font-weight-500" type="submit">{{$t('Auth.set_new_password')}}</button>
        ...
        if(valid && confirmationErr.value === ''){
            ...
            try {
                ...
                apiRequestWithoutSecure("post", env.RESETPASSWORD, reqObj).then(() => {
                    ...
                    $toast.success(t("Toast.Password_set_new_has_been_successfully"),{position: 'top-right'});
                    router.replace({ name: 'Log-in' });
                }).catch((error) => {
                    isSpinner.value = false;
                    showResend.value = true;
