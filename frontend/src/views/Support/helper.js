    ...
    function filterSprints(object = null, showArchived, filterFavorites) {
        ...
        if(object.sprintsObj && Object.keys(object.sprintsObj).length) {
            Object.values(object.sprintsObj).forEach((sprint) => {
                if(sprint.deletedStatusKey !== 1 && (companyUserDetail.value.roleType === 1 || companyUserDetail.value.roleType === 2) || (!sprint?.private || sprint?.AssigneeUserId?.includes(userId.value))) {
                    if(!showArchived) {
                        if(!sprint.deletedStatusKey) {
                            sprints[sprint.id] = sprint;
                        }
                    } else {
                        if(sprint.deletedStatusKey !== 1 && (sprint.deletedStatusKey === 2 || sprint.archiveTaskCount)) {
                            sprints[sprint.id] = sprint;
                        }
                    }
                }
            ...
            let tmp = {};
            if(filterFavorites) {
                const favourites = Object.values(sprints).filter(x => x.favouriteTasks?.length && x.favouriteTasks.filter((y) => y.userId === userId.value).length).sort((a,b) => a?.createdAt?.seconds > b?.createdAt?.seconds ? -1 : 1)
                const others = Object.values(sprints).filter(x => !x.favouriteTasks?.length || !x.favouriteTasks.filter((y) => y.userId === userId.value).length).sort((a,b) => a?.createdAt?.seconds > b?.createdAt?.seconds ? -1 : 1)
        ...
        if(object.sprintsfolders && Object.keys(object.sprintsfolders).length) {
            Object.keys(object.sprintsfolders).forEach((key) => {
                ...
                const tmpObj = JSON.parse(JSON.stringify(object.sprintsfolders[key]));
                ...
                if(!showArchived) {
                    ...
                    if(folder.deletedStatusKey !== 1 && (folder.deletedStatusKey === 2 || (folder.sprintsObj && Object.keys(folder.sprintsObj).length))) {
                        folders[key] = folder;
                    }
                }
            })
    ...
    const updatePriority = (task, newPriority,assigneeType,isUpdateTask) => {
        return new Promise((resolve, reject) => {
            try {
                ...
                let priorityObj = {
                    'statusImage' : priority.image,
                    'priorityName' : priority.value,
                    'taskId': task._id,
                    'taskName': task.TaskName,
                    'userName' : userData.Employee_Name,
                    'newStatusImage' : newPriority.image,
...
export function taskListHelper() {
    ...
    function getSprintTasks({projectId, sprintId, item, fetchNew = false, projectData ,indexName,parentId = ''}) {
        return new Promise((resolve, reject) => {
            try {
                if(permit === null && projectData.isGlobalPermission === false) {
                    return;
                }
                let customerId = route.params.cuid ? route.params.cuid : ''
                dispatch("taskData/getPaginatedTasks", {
                    cid: companyId.value,
                    pid: projectId,
                    sprintId: sprintId,
                    item,
                    ...
                    indexName: indexName,
                    parentId : parentId,
                    isSupport : route.name === 'Support' ? true : false,
    ...
    async function groupBy(type, refetch = false,project,sprintData,groupedTasks,isBoard,cb) {
        try {
            ...
            if(type === 0) {
                ...
                let checkCase = (op, key, seconds, dateFormat = false) => {
                    switch(op) {
                        ...
                        case "lt":
                            if (dateFormat) {
                                return {[key]: {$lt: new Date((seconds * 1000))}}
                            } else {
                                return {[key]: {$lt: seconds}}
                            }
                            // return `${key} :<= ${seconds + dayHrs}`

                        case "gt":
                            if (dateFormat) {
                                return {[key]: {$gt: new Date(seconds * 1000)}}
                            } else {
            ...
            if(sprints && sprints.length) {
                ...
                if(refetch === true) {
                    ...
                    sprints[0].items.forEach((item) => {
                        promises.push(
                            getSprintTasks({projectId: project.value._id, sprintId: sprints[0]?.id, item, fetchNew: true,projectData: project.value, indexName: item.indexName})
                        )
                    })
