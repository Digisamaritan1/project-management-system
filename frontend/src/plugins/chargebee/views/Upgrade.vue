<template>
    <div v-if="companyOwner.isCurrentUser" id="paymentModel" :class="{'p20px-0px' : clientWidth <=767 , 'p50px-0px' : clientWidth > 767 }">
        ...
        <div class="monthly__yearly-subcription d-flex align-items-center justify-content-center mt-30px">
            ...
            <Toggle v-model="subsPeriodType" width="100%" height="auto" :text="subscriptionPeriod" @change="(e) => {subscriptionPeriod = subsPeriodType ?'month' : 'year'}">
                <template #body>
                    <div class="p-4px">
                        <button class="toggle-option" :class="{ 'upgradeToggleActive': subscriptionPeriod === 'month' }"> {{$t('Upgrades.per_month')}}</button>
                        <button  class="toggle-option" :class="{ 'upgradeToggleActive': subscriptionPeriod === 'year' }"> {{$t('Upgrades.per_year')}}
        ...
        <div class="enterpise__main-wrapper pl-15px pr-15px" :class="{'pt-20px' : clientWidth <=767 , 'pt-40px' : clientWidth > 767 }">
            ...
            <div :class="[{'desktop-enterprise__limitness-wrapper' : clientWidth > 767,'mobile-enterprise_second-wrapper' : clientWidth <= 767}]">
                <div v-for="(data,index) in chargeBeePriceData?.filter((x) => x?.status === 1)" :key="index" :class="[{'charge__bee-pricedata': !data.defaultSubscribe}]">
                    <div class="pl-20px pr-20px pt-30px pb-20px enterprise__limitness-wrapper bg-white payment__box-shadow"  v-if="!data.defaultSubscribe">
                        ...
                        <div v-if="!data.isContactSupport" class="price__display" :class="{'pt-10px' : clientWidth <=767 , 'pt-20px' : clientWidth > 767 }"><span class="font-size-24">$</span>{{getPriceDisplay(data,subscriptionPeriod)}}<span class="d-block font-size-16 gray81 font-weight-500">{{$t('Upgrades.per_member')}}/{{$t('Upgrades.per_month')}}</span></div>
                        ...
                        <div class="blue cursor-pointer" v-if="isMuchMore" @click="() => isMuchMore = false">...{{$t('Upgrades.and_much_more')}}</div>
            ...
            <div :style="{'opacity': billingSpinner ? 0.5 : 1, 'pointer-events': billingSpinner ? 'none' : ''}">
                ...
                <div class="upgrade__mainsection-wrapper addonWrapper"  v-if="!showSomethingWentWrong && !showSuccessPlanPurchase"  :class="{'p-15px' : clientWidth <=767 , 'p-30px' : clientWidth > 767 }">
                    <div class="payment__text font-size-22 font-weight-700 black pb-20px">
                        {{$t('Upgrades.select_payment_method')}}
                    </div>
                    ...
                    <div class="buy__now-btn bg-white pb-20px">
                        <button class="font-size-16 font-weight-400 bg-blue border-radius-4-px white border-0 cursor-pointer bg-dark-green" @click="makePayment()">Pay ${{finalPrice}}</button>
                    </div>
                </div>
                <div v-if="showSuccessPlanPurchase" class="upgrade__mainsection-wrapper addonWrapper">
                    ...
                    <div class="plan__upgradesucessfully-wrapper w-100 m0-auto text-center bg-white">
                        <div>
                            ...
                            <span class="d-block font-size-16 font-weight-400 plan__upgrade-desc GunPowder pt-20px pb-20px">{{$t('Upgrades.plane_upgrade_msg')}}</span>
...
<script setup>
    ...
    const savePercentage = computed(()=>{
        if (chargeBeePriceData.value.length) {
            ...
            let monthly = price.find((x)=> x.period_unit == 'month').price || 0;
            let yearly = price.find((x)=> x.period_unit == 'year').price || 0;
    ...
    function setDefault() {
        if (chargeBeePriceData.value && chargeBeePriceData.value.length && Object.keys(subscriptionData.value).length) {  
            let currPriceid = subscriptionData.value?.subscription_items?.find((x)=> x.item_type == 'plan')?.item_price_id || '';
    ...
    function upgrade(data) {
        if (isUpdate.value) {
            ...
            document.getElementById('paymentModel').scrollIntoView({behavior: 'smooth',block: "start"});
            selectedPlan.value = data.planDetails.name;
        ...
        const getFunc = () => {
            ...
            .then((resp) => {
                if(resp.data.status == true && resp.data.isCards == true) {
                    paymentSorceList.value = resp.data.paymentSourceList;
    ...
    function makePayment() {
        ...
        if (selectedCard.value == '0') {
            ...
            createCheckout(paymentSorceList.value[Number(selectedCard.value -1)].id);
        }
    }
    function createCheckout  (paymentSourceId) {
        billingSpinner.value = true;
        selectedPrice.value = chargeBeePrice.value.itemPriceArray.find((x)=> x.period_unit == subscriptionPeriod.value)
        let addOnId = chargeBeePrice.value.addonPriceArray.find((x)=> x.period_unit == subscriptionPeriod.value)
        if(paymentSourceId && currentCompany.value && currentCompany.value?.billingDetails && Object.keys(currentCompany.value?.billingDetails).length) {
            let subscription_items = [
                {
                    item_price_id : selectedPrice.value.id
                }
            ]
            if (companyUsers.value.length > 1) {
                subscription_items.push({
                    item_price_id : addOnId.id,
                    quantity : companyUsers.value.length - 1
                });
            }
            const axiosObj = {
                ...
                planId: chargeBeePrice.value.planName,
            }
            apiRequest("post",env.CREATE_SUBSCRIPTION_CHARGEBEE,axiosObj)
                .then((resAxios) => {
                    if(resAxios.data.status) {
                        if (resAxios.data?.data?.subscription?.id) {
                            commit("settings/mutateCompanies", {
                                data: {...currentCompany.value, SubcriptionId: resAxios.data?.data?.subscription?.id},
                                op: "modified",
                            })
                        }
                ...
                .catch((error) => {
                    console.error(error);
                    $toast.error(t('Toast.Plan_not_purchased_Please_try_again'),{position: 'top-right'});
                    billingSpinner.value = false;
                })
        } else {
            $toast.error(t('Toast.Please_fill_out_billing_details_first'),{position: 'top-right'});
            isSpinner.value = false;
        }
    }
    function getDATA () {
...
.enterpise__main-wrapper .charge__bee-pricedata .price__display , 
.enterpise__main-wrapper .charge__bee-pricedata .price__display{
    font-size: 50px;
    font-weight: 600;
    letter-spacing: -2px;
...
.enterpise__main-wrapper .charge__bee-pricedata::before ,
.enterpise__main-wrapper .charge__bee-pricedata::before {
    border-radius: 12px;
    content: '';
