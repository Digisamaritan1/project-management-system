<template>
    <div class="release-note-section">
        ...
        <div class="version-main-section" v-if="!loading && !errorMessage">
            ...
            <div v-else>
                <span v-if="fromWhich === ''" class="font-size-36 font-weight-700 line-height-48_8 blue">Are you sure want to update?</span>
            </div>
            <div class="version-section" v-if="versionUpdate?.finalVersionObj?.releaseNotes?.notes?.length">
                <div class="new-version-main">
                    <div class="new-version-box">
                        <p class="m-0 black font-size-22 font-weight-700 line-height-33">Whats New {{versionUpdate.finalVersionObj.version}}</p>
                        <p class="m-0 black font-size-16 font-weight-400 line-height-24">Find out whats new included in this update</p>
                    </div>
                </div>
                <div class="new-version-body style-scroll-2-px">
                    <div>
                        <p class="m-0 mb-15px black font-size-22 font-weight-700 line-height-33">Please see below for a list of changes to the feature</p>
                        <div v-for="(raw, index) in versionUpdate?.finalVersionObj?.releaseNotes?.notes" v-bind:key="index">
                            <p class="m-0 mb-10px mt-5px black font-size-16 font-weight-500 line-height-24">Improve Perfomance (v{{ raw.title }})</p>
                            <span v-html="raw.description"></span>
                        </div>
                    </div>
                    <div v-if="versionUpdate?.finalVersionObj?.releaseNotes?.newIntegrations?.length">
                        <p class="m-0 mt-20px mb-20px black font-size-22 font-weight-700 line-height-33">Integrate new services into an alian hub</p>
