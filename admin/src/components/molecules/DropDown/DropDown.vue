...
function buttonClick(flag = false) {
    ...
    if(dropdownVisible.value === true) {
        ...
        nextTick(() => {
            ...
            const {top, left, height: eHeight, width: eWidth} = element.getBoundingClientRect();
            const {height, width} = childNode.getBoundingClientRect();

            if(document.documentElement.clientWidth < (left + eWidth + width)) {
                const offset = document.documentElement.clientWidth - (left + width + 15);
                childNode.style.left = left + offset + "px";
            } else {
                childNode.style.left = left + eWidth + "px";
            }

            if(document.documentElement.clientHeight < (top + eHeight + height)) {
                const offset = document.documentElement.clientHeight - (top + eHeight + height + 15);
                childNode.style.top = top + offset +"px";
            } else {
                if(props.keepSameWidth){
                    let widthCheck = document.getElementsByClassName(`drop-down-menu`)[0];
                    widthCheck.style.width = eWidth + "px"
                }
                const position = top + (eHeight < 25 ? 25 : eHeight)                    
